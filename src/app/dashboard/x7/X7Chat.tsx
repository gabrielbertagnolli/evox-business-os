...
import { useX7Chat, type X7Message, type X7Summary } from "@/hooks/api/useX7Chat";
...
function MessageBubble({ message }: { message: X7Message }) {
...
export default function X7Chat() {
          ...
          {chatMutation.isPending && (
            <div className="flex justify-start">
              <div className="flex items-center gap-2 rounded-2xl px-4 py-3 text-sm text-white/45" style={{ background: "rgba(255,255,255,0.045)", border: "1px solid rgba(255,255,255,0.08)" }}>
                <Loader2 size={14} className="animate-spin text-[#2d7bff]" /> X7 está consultando tus fuentes…
              </div>
            </div>
          )}
        ...
        <div className="border-t border-white/10 p-5">
          ...
          <form onSubmit={handleSubmit} className="flex items-end gap-3">
            <textarea
              ...
              placeholder="Pregúntale a X7 qué hacer con tus fuentes conectadas…"
              rows={2}
          ...
          <div className="mt-5 space-y-2">
            {visibleSources.length > 0 ? (
              ...
              <p className="rounded-xl px-3 py-3 text-sm text-white/35" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                Envía un mensaje para que X7 cargue el contexto de tus fuentes configuradas.
              </p>
            )}
          </div>
        </div>
